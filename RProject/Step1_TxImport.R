...
library(datapasta) # great for copy/paste data in the R environment 
...
# this renaming of tx_id to target_id is a 1:1 mapping and is essential to map correctly
Tx <- dplyr::rename(Tx, target_id = tx_id)
# transcrip ID needs to be the first column in the dataframe
# this takes the columns we are interested in
Tx <- dplyr::select(Tx, "target_id", "gene_name")
# you can make the decision to collapse at the gene or transcript level
...
Txi_gene <- tximport(path, 
                     type = "kallisto", 
                     tx2gene = Tx, 
                     txOut = FALSE, # How does the result change if this =FALSE(gene) vs =TRUE(transcripts)?
                     countsFromAbundance = "lengthScaledTPM",
                     ignoreTxVersion = TRUE) # ignore transcript version
beep(sound = 2)
# note comment about h5 files are related to bootstrapping, ignore, kallisto is changing
